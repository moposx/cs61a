...
class Mint:
    ...
    def create(self, kind):
        assert issubclass(kind, Coin), 'Must use a valid type of coins.'
...
def is_bst(t):
    ...
    # selectors to obtain the extreme values of every binary tree.
    def bst_min(t):
        return t.label if t.is_leaf() else min([t.label] +
                                               list(map(bst_min, t.branches)))

    def bst_max(t):
        return t.label if t.is_leaf() else max([t.label] +
                                               list(map(bst_max, t.branches)))
...
def path_yielder(t, value):
    ...
    def find_path(t, value, known_path):
        nonlocal paths

        # record the current path
        current_path = known_path[:]  # copy the array first! :(
        current_path.append(t.label)

        if t.label == value:
            paths.append(current_path)

        # leaf node judgement should postdate
        # the node label's comparison with the target value
        if t.is_leaf():
            return

        for branch in t.branches:
            find_path(branch, value, current_path)
