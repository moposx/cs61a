...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while p is not beehive:
            bee = rANTdom_else_none(p.bees)

            if bee and self.min_range <= p_distance <= self.max_range:
                return bee
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        damage = self.damage + amount if amount >= self.armor else amount
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        # BEGIN Problem 6
        if self.digesting > 0 and HungryAnt.time_to_digest != 0:  # the ant is digesting
            self.digesting -= 1
        else:
            bee = rANTdom_else_none(self.place.bees)

            if bee:
                Bee.reduce_armor(bee, bee.armor)
                self.digesting = 3
        # END Problem 6
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def __init__(self, armor=1):
        # BEGIN Problem EC
        self.is_true_queen = not bool(QueenAnt.quantity)
        self.is_impostor_queen = not self.is_true_queen
    ...
    def action(self, gamestate):
        ...
        if self.is_true_queen:
            ...
            while place:
                if place.ant and not place.ant.inspired_by_queen:
                    place.ant.damage *= 2
                    place.ant.inspired_by_queen = True

                place = place.exit

            ThrowerAnt.action(self, gamestate)

        elif self.is_impostor_queen:
