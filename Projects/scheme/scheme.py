...
from curses import use_env  # Python 2 compatibility
...
def scheme_eval(expr, env, _=None):  # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # operators are instances of Procedure.
        validate_procedure(expr_operator)

        # apply the procedure with the above operands and the specified environment.
...
class LambdaProcedure(Procedure):
    ...
    def make_call_frame(self, args, env):
        ...
        # BEGIN PROBLEM 11
        # use self.env, as it is where the lambda procedure is defined.
        frame = Frame(self.env)

        formal = self.formals
        actual = args

        while formal and actual:
            frame.define(formal.first, actual.first)
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        # note that expressions, as a list, has a length of 2.
        value = scheme_eval(expressions.rest.first, env)

        # bind value to target in the given environment
        ...
        # we need to construct a new pair so the procedure can be parsed
        procedure = do_lambda_form(Pair(target.rest, expressions.rest), env)
        env.define(fn_name, procedure)
...
def do_and_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    # simplest case: `(and)` with no operands
    if not expressions:
        return True
...
def do_or_form(expressions, env):
    ...
    """
    # BEGIN PROBLEM 12
    # simplest case: `(or)` with no operands
    if not expressions:
        return False
...
def do_cond_form(expressions, env):
    ...
    while expressions is not nil:
        ...
        if is_true_primitive(test):
            # BEGIN PROBLEM 13
            return test if not clause.rest else eval_all(clause.rest, env)
            # END PROBLEM 13
        expressions = expressions.rest
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        ...
        validate_form(binding, 2, 2)  # name to value, strictly 2 components

        formal = binding.first
        actual = scheme_eval(binding.rest.first, env)
        names = Pair(formal, names)
        values = Pair(actual, values)
