...
def prune_min(t):
    ...
    if t.is_leaf():
        ...
        left_label = t.branches[0].label
        right_label = t.branches[1].label
...
class CheckingAccount(Account):
    ...
    def deposit_check(self, check_obj):
        if check_obj.payable_to == self.holder and not check_obj.deposited:
            check_obj.deposited = True
            return self.deposit(check_obj.amount)
        else:
...
def foldl(link, fn, z):
    ...
    result = fn(z, link.first)

    return foldl(link.rest, fn, result)
...
def filterl(lst, pred):
    ...
    # the fn should receive the first and rest part of a Link
    # depending on the predicate, create new Link (with?) the first part.
    fn = lambda first, rest: Link(first, filterl(rest, pred)) if pred(first) \
                else filterl(rest, pred)
...
def reverse(lst):
    ...
    True
    """
    if not lst or not lst.rest:
        return lst

    fn = lambda first, rest: Link(rest, first)
