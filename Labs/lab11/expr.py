...
class Name(Expr):
    ...
    def eval(self, env):
        ...
        None
        """
        assert type(env) is dict, 'env must be a dictionary.'
...
class CallExpr(Expr):
    ...
    def eval(self, env):
        ...
        Number(14)
        """
        self_operator = self.operator.eval(env)
        self_operands = [operand.eval(env) for operand in self.operands]

        return self_operator.apply(self_operands)
...
class LambdaFunction(Value):
    ...
    def apply(self, arguments):
        ...
        parent_env = self.parent.copy()

        # bind actual parameters to formal parameters
        for formal, actual in zip(self.parameters, arguments):
            parent_env[formal] = actual

        # evaluate the body
        return self.body.eval(parent_env)
